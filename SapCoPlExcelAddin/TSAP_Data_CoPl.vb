...
Public Class TSAP_Data_CoPl
    ...
    Public Sub New(ByRef pPar As SAPCommon.TStr, ByRef pIntPar As SAPCommon.TStr, ByRef pSAPCostActivityPlanning As SAPCostActivityPlanning, pFunction As String)
        aPar = pPar
        aIntPar = pIntPar
        aFunction = pFunction
        ...
        ' get Metadata
        If pFunction = "postPC" Then
            pSAPCostActivityPlanning.getMeta_PostPrimCost(aParamDic, aStrucDic)
        ElseIf pFunction = "postAO" Then
            pSAPCostActivityPlanning.getMeta_PostActivityOutput(aParamDic, aStrucDic)
        ElseIf pFunction = "postAI" Then
            pSAPCostActivityPlanning.getMeta_PostActivityInput(aParamDic, aStrucDic)
        ElseIf pFunction = "postSK" Then
            pSAPCostActivityPlanning.getMeta_PostKeyFigure(aParamDic, aStrucDic)
        End If
    End Sub
    ...
    Public Function fillData(pData As TData) As Boolean
        ...
        For Each aKvB In pData.aTDataDic
            ...
            For Each aTStrRec In aTDataRec.aTDataRecCol
                ...
                End If
            Next
            aNewTStrRec = New SAPCommon.TStrRec("INDEXSTRUCTURE", "OBJECT_INDEX", CStr(aCnt))
            aDataDic.addValue(CStr(aCnt), aNewTStrRec)
            aNewTStrRec = New SAPCommon.TStrRec("INDEXSTRUCTURE", "VALUE_INDEX", CStr(aCnt))
            aDataDic.addValue(CStr(aCnt), aNewTStrRec)
            aNewTStrRec = New SAPCommon.TStrRec("COOBJECT", "OBJECT_INDEX", CStr(aCnt))
            aDataDic.addValue(CStr(aCnt), aNewTStrRec)
            If aDataDic.aTDataDic.ContainsKey("PERVALUE") Then
                aNewTStrRec = New SAPCommon.TStrRec("PERVALUE", "VALUE_INDEX", CStr(aCnt))
                aDataDic.addValue(CStr(aCnt), aNewTStrRec)
            End If
            If aDataDic.aTDataDic.ContainsKey("TOTVALUE") Then
                aNewTStrRec = New SAPCommon.TStrRec("TOTVALUE", "VALUE_INDEX", CStr(aCnt))
                aDataDic.addValue(CStr(aCnt), aNewTStrRec)
            End If
            If aDataDic.aTDataDic.ContainsKey("CONTRL") Then
                aNewTStrRec = New SAPCommon.TStrRec("CONTRL", "ATTRIB_INDEX", CStr(aCnt))
                aDataDic.addValue(CStr(aCnt), aNewTStrRec)
                aNewTStrRec = New SAPCommon.TStrRec("INDEXSTRUCTURE", "ATTRIB_INDEX", CStr(aCnt))
                aDataDic.addValue(CStr(aCnt), aNewTStrRec)
            End If
            aCnt += 1
        Next
